...
<head>
			...
			success : function(text)
			{
				...
				if(json.list!=null && json.list.length != 0)
				{
					...
					for(var i=0; i<json.list.length; i++){
						var com = json.list[i];
						var name = com.name;
						var lineDiv = $("<div style='clear:both;'></div>");
						var itemDiv1 = $("<div class='item'></div>");
						...
						//td.appendTo(tr);
						//tr.appendTo(itemTbl);
						//itemTbl.appendTo(itemDiv);

						img.appendTo(itemDiv2);

						itemDiv1.appendTo(lineDiv);
						itemDiv2.appendTo(lineDiv);
						//itemDiv3.appendTo(lineDiv);
						lineDiv.appendTo(listDiv);

						//itemDiv3.bind("click",{key:i},getStat);
						img.bind("click",{key:i},sendWol);
	...
	function sendWol(event){
		...
		$.ajax({
			...
			success : function(text)
			{
				alert("WOL 신호를 성공적으로 보냈습니다.");
				//eval("var json = "+text);
				//alert(json.result[0].mp3); 
				/*if(json.stat!=null)
				{
					if(json.stat=="ON"){
						$("#stat_"+key).css("background-color","green");
					}else if(json.stat=="OFF"){
						$("#stat_"+key).css("background-color","red");
					}
